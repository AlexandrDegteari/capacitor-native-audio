...
//                    }
    ...
    fun updateQueue(call: PluginCall) {
        queueHandler.postTask {
            try {
                ...
                queue.updateQueue(jsTracks = jsTracks.toList()) {
                    call.resolve()
                }
            } catch (ex: Exception) {
                call.reject(ex.message)
    ...
    fun notifyPause(time: Double, duration: Double, queueId: String, trackId: String, index: Int) {
        val jsObject = JSObject()
        jsObject.put("time", time)
        jsObject.put("duration", duration)
        jsObject.put("id", queueId)
...
//                    }
//                }
                val queueTrack = QueueTrack(audioId ?: "", assetPath!!, assetPath, false, false)
